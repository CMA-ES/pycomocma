...
class CoMoCmaes(object):
    ...
    def __init__(self,
        ...
        # self.front is the list of the objective values of the non-dominated kernel means 
        # by non-dominated, we mean non-dominated by other current kernels means
        self.front = NDA([kernel.fit.fitnesses for kernel in self.kernels],
                         reference_point)
            ...
            for _ in range(self.inner_iterations):
              ...
              # in this case it does not converge
               ...
               if fit in self.front:
                ...
                # updating the fitness and add it to self.front
               kernel.fit.fitnesses = self.evaluate(kernel.mean)
               self.front.add(kernel.fit.fitnesses)

        # store the data
        self.hv += [self.front.hypervolume]
        self.hv_archive += [self.archive.hypervolume]
        self.ratio_nondominated_kernels += [len(self.front)/self.num_kernels]
